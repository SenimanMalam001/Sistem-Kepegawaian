...
$(document).ready(function(){
  ...
  function show_product(){
    $.ajax({
      type  : 'ajax',
      url   : '<?php echo base_url() . 'C_Surat_Tugas/st_data' ?>',
      async : false,
      dataType : 'json',
      success : function(data){
        ...
        for(var i=0; i<data.length; i++){
          ...
          if(data[i].sk_fileurl){
            $berkas = '<a href="<?php echo base_url() . 'files/surat_tugas/'; ?>' + data[i].sk_fileurl + '" class="btn btn-success btn-sm data-id="'+data[i].sk_fileurl+'" download><i class="fa fa-download" aria-hidden="true"></i> Download </a>' + ' ';
          }
            ...
            html += '<tr>'+
                ...
                '<td style="text-align:right;">'+
                    ...
                    '" data-maksud="'+data[i].maksud_perjalanan_dinas+
                    '" data-tempat="'+data[i].tempat_tujuan+
  ...
  function get_rubrik_data(){
    $.ajax({
      type  : 'ajax',
      url   : '<?php echo base_url() . 'C_Surat_Tugas/get_rubrik_data' ?>',
      async : false,
      dataType : 'json',
      success : function(data){
        var html = '';
  ...
  $('#tab_keanggotaan').on('click', function(){
    ...
    $.ajax({
      type : "POST",
      url   : '<?php echo base_url() .'C_Surat_Tugas/getRubrikKategori'?>',
      async : false,
  ...
  function getAnggotaSK(id_sk){
    $.ajax({
      type: "POST",
      url   : '<?php echo base_url() . 'C_Surat_Tugas/getAnggotaSK' ?>',
      async : false,
      data : {id_sk, id_sk},
      dataType : 'json',
      success : function(data){
  ...
  function insertingSK(id){
    var id = $('[name=id]').val();
    var tgl_awal = $('#tgl_awal').val();
    var tgl_akhir = $('#tgl_akhir').val();
    var deskripsi = $('#deskripsi').val();
    var sk_fileurl	= $('#sk_fileurl').val();
    var tempat_tujuan = $('#tempat_tujuan').val();
    var maksud_perjalanan = $('#maksud_perjalanan').val();
    

    console.log('Data : ' + tgl_awal + tgl_akhir + deskripsi + sk_fileurl + tempat_tujuan + maksud_perjalanan);

    $.ajax({
        type : "POST",
        url  : '<?php echo base_url() . 'C_Surat_Tugas/save'?>',
        dataType : "JSON",
        data : {id:id, tgl_awal:tgl_awal, tgl_akhir:tgl_akhir, deskripsi:deskripsi, sk_fileurl:sk_fileurl, tempat_tujuan:tempat_tujuan, maksud_perjalanan:maksud_perjalanan},
        success: function(data){
          console.log('Berehasil Insert surat tugas : ' + data);

          // Setelah Insert SK insert member nya
  ...
  function insertingSKMember(id_sk, nip_pegawai, id_rubrik){
    ...
    console.log(tempat_tujuan);
    
    
    $.ajax({
        type : "POST",
        url  : '<?php echo base_url() . 'C_Surat_Tugas/save_member' ?>',
        dataType : "JSON",
        data : {id_sk:id_sk, nip_pegawai:nip_pegawai, id_rubrik:id_rubrik},
        success: function(data){
          console.log('Berehasil Insert : ' + data);
        },
        error: function(jqXHR, textStatus, errorThrown){
  ...
  $('#show_data').on('click','#show_edit_data', function(){
    ...
    var nama_tugas = $(this).data('deskripsi');
    ...
    $('[name="maksud_tujuan_edit"]').val(maksud);
    $('[name="tempat_tujuan_edit"]').val(tempat);
    $('[name="tgl_akhir_edit"]').val(tgl_akhir);
    $('[name="nama_tugas_edit"]').val(nama_tugas);
  ...
  $('#btn_update').on('click', function(){
    ...
    var maksud = $('[name="maksud_tujuan_edit"]').val();
    var tempat = $('[name="tempat_tujuan_edit"]').val();

    console.log('Data : ' + id + ',' + tgl_awal + ',' + tgl_akhir + ',' + maksud +',' + tempat);

    $.ajax({
      type : "POST",
      url : '<?php echo base_url() . 'C_Surat_Tugas/update' ?>',
      dataType : "JSON",
      data : {id:id, tgl_awal:tgl_awal, tgl_akhir:tgl_akhir, deskripsi:deskripsi, maksud:maksud, tempat:tempat},
      success : function(data) {
  ...
  $('#btn_delete').on('click', function(){
    ...
    $.ajax({
      type : "POST",
      url : '<?php echo base_url() . 'C_Surat_Tugas/delete'?>',
      dataType : "JSON",
      data : {id:id},
      success: function(data){
        $('#myForm')[0].reset();
  ...
  $('#show_data').on('click', '#show_anggota_sk', function(){
    ...
    $('[name="keanggotaan_sk_edit"]').val(id_sk);
    ...
    $.ajax({
      type : "POST",
      url   : '<?php echo base_url() .'C_Surat_Tugas/getIdRubrikByIdSK'?>',
      async : false,
      data : {id_sk: id_sk},
      dataType : 'json',
      success : function(data){
  ...
  function addMember(id_sk, nip, peran){
    ...
    $.ajax({
      type : "POST",
      url   : '<?php echo base_url() .'C_Surat_Tugas/addMember'?>',
      async : false,
  ...
  console.log('hapus anggota di edit');
    ...
    $.ajax({
      type : "POST",
      url   : '<?php echo base_url() .'C_Surat_Tugas/removeMember'?>',
      async : false,
